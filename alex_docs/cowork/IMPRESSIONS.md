...
**The approval gating is enterprise-ready.** Per-action approve/cancel with action-specific button labels (Send, Post, Create), "Don't ask again" for trusted operations, "Approve All" for batch approvals, rich content previews before sending, risk-level classification, and the ability to pause at any point: this is what IT departments need to say yes. VS Code's approach (blanket auto-run toggle) looks crude by comparison.
