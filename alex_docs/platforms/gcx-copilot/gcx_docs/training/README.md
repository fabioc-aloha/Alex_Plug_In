# Training Materials

> Hands-on workshops and exercises for mastering the GCX Copilot.

---

## Training Tracks

| Track | Duration | Audience | Description |
|-------|----------|----------|-------------|
| [Fundamentals](fundamentals.md) | 2 hours | All users | Core concepts and basic usage |
| [Code Assistant](code-assistant.md) | 3 hours | Developers | Code review, debugging, testing |
| [Technical Writer](technical-writer.md) | 2 hours | All users | Documentation generation |
| [Documentation Wizard](doc-wizard.md) | 2 hours | Doc owners | Quality audits and maintenance |
| [Integration Hub](integration-hub.md) | 4 hours | Developers | Platform integrations deep dive |

---

## Workshop Format

Each workshop includes:

1. **Concepts** — Key ideas and mental models
2. **Walkthrough** — Guided demonstration
3. **Exercises** — Hands-on practice
4. **Challenge** — Real-world scenario
5. **Reference** — Quick lookup card

---

## Fundamentals Track (2 hours)

### Module 1: Introduction (30 min)

- What is the GCX Copilot?
- The Four Pillars model
- How it enhances Copilot

### Module 2: Setup & Configuration (30 min)

- Prerequisites and installation
- VS Code settings
- Verifying your setup

### Module 3: Basic Usage (30 min)

- Asking effective questions
- Using slash commands
- Understanding responses

### Module 4: Best Practices (30 min)

- Providing context
- Choosing the right pillar
- Trust but verify

---

## Certification Path

| Level | Requirements | Badge |
|-------|--------------|-------|
| **GCX Agent User** | Complete Fundamentals track | 🥉 |
| **GCX Agent Developer** | Complete Code Assistant + Integration Hub | 🥈 |
| **GCX Agent Expert** | All tracks + contribute 1 skill | 🥇 |

---

## Self-Paced Resources

- [Video Tutorials](videos/README.md) — Coming soon
- [Exercise Repository](exercises/README.md) — Hands-on labs
- [Cheat Sheets](cheatsheets/README.md) — Quick reference cards

---

## Schedule a Workshop

Contact the GCX Copilot team to schedule a facilitated workshop for your team.

- **Email**: gcx-ai-agent@microsoft.com
- **Teams**: [GCX Copilot Support]
