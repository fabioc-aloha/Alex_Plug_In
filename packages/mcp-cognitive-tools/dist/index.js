...
const TOOLS = [
    {
        ...
        description: "Check the health of Alex's cognitive architecture - validates synapses, memory files, and connections",
        inputSchema: {
            type: "object",
            properties: {
                workspacePath: {
                    type: "string",
                    description: "Path to workspace with .github/ directory (optional, uses cwd if not specified)",
                },
            },
        },
    },
    {
        ...
        description: "Search across all Alex memory systems (skills, instructions, prompts, episodic, global knowledge)",
        inputSchema: {
            type: "object",
            properties: {
                query: {
                    type: "string",
                    description: "Search query - matches against file names, descriptions, and content",
                },
                memoryType: {
                    type: "string",
                    enum: [
    ...
    },
    {
        ...
        description: "Get current inventory of Alex's cognitive architecture - skill counts, trifectas, agents, etc.",
        inputSchema: {
            type: "object",
            properties: {
                workspacePath: {
...
const PROMPTS = [
    {
        name: "health-check",
        description: "Run a cognitive architecture health check and report status",
    },
    {
        name: "architecture-overview",
        description: "Get a summary of how Alex's cognitive architecture is structured in this workspace",
    },
    {
        name: "search-knowledge",
        description: "Search Alex's knowledge base for patterns, insights, or prior learnings",
        arguments: [
            {
                name: "query",
    ...
    {
        name: "save-insight",
        description: "Save a new insight or lesson learned to Alex's global knowledge base",
        arguments: [
            {
                name: "title",
                description: "Brief title for the insight",
...
// ---------------------------------------------------------------------------
function getArchitectureResources() {
...
async function synapseHealth(workspacePath) {
    const basePath = workspacePath || process.cwd();
    const githubPath = path.join(basePath, ".github");
    if (!fs.existsSync(githubPath)) {
        return JSON.stringify({
            status: "error",
            message: "No .github directory found. Alex cognitive architecture not installed.",
        });
    }
    const stats = {
        skills: 0,
...
async function architectureStatus(workspacePath) {
    const basePath = workspacePath || process.cwd();
    const githubPath = path.join(basePath, ".github");
    if (!fs.existsSync(githubPath)) {
        return JSON.stringify({
...
async function knowledgeSearch(query, category = "all", limit = 10) {
    const results = [];
    const lowerQuery = query.toLowerCase();
    if (!fs.existsSync(GLOBAL_KNOWLEDGE_PATH)) {
        return JSON.stringify({
...
async function knowledgeSave(title, content, category, tags = []) {
        ...
        try {
            ...
            switch (name) {
                case "alex_synapse_health":
                    result = await synapseHealth(args?.workspacePath);
                    break;
                case "alex_memory_search":
                    result = await memorySearch(args?.query, args?.memoryType, args?.limit);
                    break;
                case "alex_architecture_status":
                    result = await architectureStatus(args?.workspacePath);
                    break;
                case "alex_knowledge_search":
                    result = await knowledgeSearch(args?.query, args?.category, args?.limit);
                    break;
                case "alex_knowledge_save":
                    result = await knowledgeSave(args?.title, args?.content, args?.category, args?.tags);
                    break;
                default:
