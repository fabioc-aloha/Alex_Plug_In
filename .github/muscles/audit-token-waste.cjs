...
function finding(severity, file, pattern, message, fixable) {
  findings.push({ severity, file: path.relative(base, file), pattern, message, fixable: !!fixable });
}
...
for (const f of instrFiles) {
  ...
  totalInstrLines += lc;
  if (lc > 150) { oversizedInstr.push({ file: path.basename(f), lines: lc }); }
}

for (const f of promptFiles) {
  const lc = countLines(f);
  totalPromptLines += lc;
  if (lc > 80) { oversizedPrompts.push({ file: path.basename(f), lines: lc }); }
}
...
for (const d of skillDirs) {
  ...
  if (fs.existsSync(skillFile)) {
    ...
    totalSkillLines += lc;
    if (lc > 400) { oversizedSkills.push({ file: d, lines: lc }); }
  }
}

if (!JSON_MODE) {
  console.log(`Instructions: ${instrFiles.length} files, ${totalInstrLines} total lines`);
  console.log(`Prompts:      ${promptFiles.length} files, ${totalPromptLines} total lines`);
  console.log(`Skills:       ${skillDirs.length} dirs, ${totalSkillLines} total lines`);
  if (oversizedInstr.length) {
    console.log(`\nOversized instructions (>150 lines):`);
    for (const o of oversizedInstr) { console.log(`  ${o.lines}  ${o.file}`); }
  }
  if (oversizedPrompts.length) {
    console.log(`\nOversized prompts (>80 lines):`);
    for (const o of oversizedPrompts) { console.log(`  ${o.lines}  ${o.file}`); }
  }
  if (oversizedSkills.length) {
    console.log(`\nOversized skills (>400 lines):`);
    for (const o of oversizedSkills) { console.log(`  ${o.lines}  ${o.file}`); }
  }
}
...
for (const f of instrFiles) {
  ...
  if (fs.existsSync(skillPath)) {
    ...
    if (instrLines > 50) {
      finding('WARN', f, 'overlap', `${instrLines} lines with matching skill (target: <50)`, false);
      overlapCount++;
      if (!JSON_MODE) { console.log(`  ${instrLines}  ${path.basename(f)} -> skill exists`); }
    }
  }
}
if (!JSON_MODE && overlapCount === 0) { console.log('  All overlapping instructions are <=50 lines. Clean!'); }

// ── Phase 3: Waste Pattern Scan ───────────────────────────────
if (!JSON_MODE) { console.log('\n--- Waste Pattern Scan ---'); }

const PATTERNS = [
  { name: 'Mermaid %%{init}', regex: /%%\{init/g, fixable: true, fix: line => null },
  { name: 'Azure AD (not Entra ID)', regex: /Azure AD(?! →)/g, fixable: true, fix: line => line.replace(/Azure AD/g, 'Microsoft Entra ID') },
  { name: 'Meta-block Classification:', regex: /^\*\*Classification\*\*:/gm, fixable: false },
  { name: 'Meta-block Activation:', regex: /^\*\*Activation\*\*:/gm, fixable: false },
  { name: 'Meta-block Priority:', regex: /^\*\*Priority\*\*:/gm, fixable: false },
];
...
const patternCounts = {};
for (const p of PATTERNS) { patternCounts[p.name] = 0; }
      ...
      if (FIX_MODE && p.fixable && p.fix) {
        ...
        for (const line of lines) {
          if (p.regex.test(line)) {
            ...
            if (result === null) { changed = true; continue; } // Delete line
...
for (const d of skillDirs) {
  ...
  if (fs.existsSync(skillFile) && fs.existsSync(synFile)) {
    ...
    if (/^## Synapses/m.test(content)) {
      finding('WARN', skillFile, 'inline-synapses', 'Has inline ## Synapses AND synapses.json', true);
      inlineSynapseCount++;
      if (!JSON_MODE) { console.log(`  ${d}/SKILL.md: has inline ## Synapses`); }
      ...
      if (FIX_MODE) {
        ...
        for (let i = 0; i < lines.length; i++) {
          if (/^## Synapses/.test(lines[i])) { synStart = i; }
          else if (synStart >= 0 && i > synStart && /^## /.test(lines[i]) && !/^## Synapses/.test(lines[i])) {
            synEnd = i; break;
          }
        }
        if (synStart >= 0) {
...
if (!JSON_MODE && inlineSynapseCount === 0) { console.log('  Clean — no inline synapses found.'); }
...
for (const f of instrFiles) {
  ...
  if (!fm.includes('applyTo:')) {
    ...
    if (lines > 30) {
      noApplyToCount++;
      if (!JSON_MODE) { console.log(`  ${lines}  ${path.basename(f)}`); }
    }
  }
}
if (!JSON_MODE) {
  console.log(`  ${noApplyToCount} instructions >30 lines without applyTo (body loaded at agent discretion)`);
}
...
for (const d of skillDirs) {
  ...
  if (fs.existsSync(skillFile)) {
    ...
    if (m) { skillDescChars += m[1].length; }
  }
}

const ciFile = path.join(ghDir, 'copilot-instructions.md');
const ciChars = fs.existsSync(ciFile) ? fs.readFileSync(ciFile, 'utf8').length : 0;

if (!JSON_MODE) {
  console.log(`  Instruction descriptions: ~${Math.round(descChars / 4)} tokens (${descChars} chars)`);
  console.log(`  Skill descriptions:       ~${Math.round(skillDescChars / 4)} tokens (${skillDescChars} chars)`);
  console.log(`  copilot-instructions.md:  ~${Math.round(ciChars / 4)} tokens (${ciChars} chars)`);
  console.log(`  Estimated always-on:      ~${Math.round((descChars + skillDescChars + ciChars) / 4)} tokens per request`);
}
...
if (JSON_MODE) {
  console.log(JSON.stringify({
    baseline: {
      instructions: { count: instrFiles.length, totalLines: totalInstrLines, oversized: oversizedInstr },
      prompts: { count: promptFiles.length, totalLines: totalPromptLines, oversized: oversizedPrompts },
      skills: { count: skillDirs.length, totalLines: totalSkillLines, oversized: oversizedSkills },
    },
  ...
  if (FIX_MODE) { console.log(`  Auto-fixed: ${fixCount} files`); }
