...
async function getGitHubSession() {
  try {
    const session = await vscode4.authentication.getSession("github", ["gist"], { createIfNone: true });
    return session;
  } catch (err) {
...
function getSyncMetadataPath() {
  return path4.join(getGlobalKnowledgePath("root"), "sync-metadata.json");
}
async function loadSyncMetadata() {
  const metaPath = getSyncMetadataPath();
  try {
    if (await fs4.pathExists(metaPath)) {
      return await fs4.readJson(metaPath);
    }
  } catch (err) {
  }
  return {};
...
async function saveSyncMetadata(metadata) {
  const metaPath = getSyncMetadataPath();
  await fs4.writeJson(metaPath, metadata, { spaces: 2 });
}
async function findKnowledgeGist() {
  ...
  if (await fs4.pathExists(indexPath)) {
    try {
      const localIndex = await fs4.readJson(indexPath);
...
async function pushToCloud() {
  try {
    ...
    for (const entry of localIndex.entries) {
      if (await fs4.pathExists(entry.filePath)) {
        const content = await fs4.readFile(entry.filePath, "utf-8");
        const filename = path4.basename(entry.filePath);
  ...
  try {
    ...
    for (const entry of remoteIndex.entries) {
      ...
      if (gistFile) {
        ...
        const localPath = path4.join(getGlobalKnowledgePath(subdir), filename);
        entry.filePath = localPath;
        await fs4.writeFile(localPath, gistFile.content, "utf-8");
        downloadedCount++;
      }
    }
...
async function syncWithCloud() {
  try {
    ...
    let localIndex;
    if (await fs4.pathExists(indexPath)) {
      localIndex = await fs4.readJson(indexPath);
    } else {
    ...
    if (gist) {
      for (const entry of remoteIndex.entries) {
        ...
        const subdir = entry.type === "pattern" ? "patterns" : "insights";
        const localPath = path4.join(getGlobalKnowledgePath(subdir), filename);
        if (gistFile && !await fs4.pathExists(localPath)) {
          await fs4.writeFile(localPath, gistFile.content, "utf-8");
...
`;
    ...
    return new vscode4.LanguageModelToolResult([
      new vscode4.LanguageModelTextPart(response)
    ]);
  }
};
function registerCloudSyncTools(context) {
  context.subscriptions.push(
    vscode4.lm.registerTool("alex_cloud_sync", new CloudSyncTool())
  );
}
...
function getUnconsciousChannel() {
  if (!unconsciousChannel) {
    unconsciousChannel = vscode4.window.createOutputChannel("Alex Unconscious Mind");
  }
...
function getAlexGlobalPath() {
  return path5.join(os.homedir(), ALEX_GLOBAL_HOME);
}
function getGlobalKnowledgePath(subpath) {
  return path5.join(os.homedir(), GLOBAL_KNOWLEDGE_PATHS[subpath]);
}
...
async function updateGlobalKnowledgeIndex(updater) {
  ...
  return await withFileLock(indexPath, async () => {
    ...
    await fs5.writeJson(indexPath, index, { spaces: 2 });
    return index;
  });
}
...
async function ensureGlobalKnowledgeIndex() {
  ...
  return await withFileLock(indexPath, async () => {
    ...
    await fs5.writeJson(indexPath, newIndex, { spaces: 2 });
    return newIndex;
  });
}
...
async function registerCurrentProject() {
  ...
  const dkPattern = new vscode5.RelativePattern(workspaceFolders[0], ".github/domain-knowledge/*.md");
  const dkFiles = await vscode5.workspace.findFiles(dkPattern);
...
async function createGlobalPattern(title, content, category, tags, sourceProject) {
  ...
  const filePath = path5.join(getGlobalKnowledgePath("patterns"), filename);
...
async function updateGlobalPattern(existingEntry, newContent, category, tags, sourceProject) {
  ...
  if (!filePath || !await fs5.pathExists(filePath)) {
...
async function createGlobalInsight(title, content, category, tags, sourceProject, problemContext, solution) {
  ...
  const filename = `${id}.md`;
  const filePath = path5.join(getGlobalKnowledgePath("insights"), filename);
  const fileContent = `# ${title}
  ...
  for (const entry of index.entries) {
    ...
    if (relevance > 0) {
      ...
      if (await fs5.pathExists(entry.filePath)) {
        try {
          content = await fs5.readFile(entry.filePath, "utf-8");
          for (const word of queryWords) {
...
async function promoteToGlobalKnowledge(localFilePath, category, additionalTags = []) {
  try {
    const content = await fs5.readFile(localFilePath, "utf-8");
    const filename = path5.basename(localFilePath, ".md");
    ...
    const workspaceFolders = vscode5.workspace.workspaceFolders;
    const sourceProject = workspaceFolders ? path5.basename(workspaceFolders[0].uri.fsPath) : void 0;
...
async function evaluateDKFile(filePath) {
  ...
  if (alreadyPromoted && existingEntry) {
    try {
      const localStats = await fs5.stat(filePath);
...
async function autoPromoteDuringMeditation(workspacePath, options = {}) {
  ...
  const dkPath = path5.join(workspacePath, ".github", "domain-knowledge");
  if (!await fs5.pathExists(dkPath)) {
    return result;
  }
  const files = await fs5.readdir(dkPath);
  const dkFiles = files.filter((f) => f.startsWith("DK-") && f.endsWith(".md"));
  for (const file of dkFiles) {
    const filePath = path5.join(dkPath, file);
...
var PromoteKnowledgeTool = class {
  async prepareInvocation(options, token) {
    return {
      invocationMessage: `Promoting ${path5.basename(options.input.filePath)} to global knowledge`,
      confirmationMessages: {
...
This will make it searchable and available across all your projects.`
  ...
  async invoke(options, token) {
    ...
    if (!await fs5.pathExists(filePath)) {
      return new vscode5.LanguageModelToolResult([
        new vscode5.LanguageModelTextPart(`\u274C File not found: ${filePath}`)
      ]);
    }
    const entry = await promoteToGlobalKnowledge(
      filePath,
    ...
    if (!entry) {
      return new vscode5.LanguageModelToolResult([
        new vscode5.LanguageModelTextPart(`\u274C Failed to promote file to global knowledge.`)
      ]);
    }
    triggerPostModificationSync();
...
function registerGlobalKnowledgeTools(context) {
  context.subscriptions.push(
    vscode5.lm.registerTool("alex_global_knowledge_search", new GlobalKnowledgeSearchTool()),
    vscode5.lm.registerTool("alex_save_insight", new SaveInsightTool()),
    vscode5.lm.registerTool("alex_promote_knowledge", new PromoteKnowledgeTool()),
    vscode5.lm.registerTool("alex_global_knowledge_status", new GlobalKnowledgeStatusTool())
  );
}
...
Recommendations: ${report.recommendations.length}`;
  ...
  if (action === "View Report") {
    ...
    const doc = await vscode6.workspace.openTextDocument(report.sessionFile);
...
async function scanSynapseHealth(workspaceFolder, report) {
  ...
  const synapseRegex = /\[([^\]]+\.md)\]\s*\(([^,)]+)(?:,\s*([^,)]+))?(?:,\s*([^)]+))?\)\s*-\s*"([^"]*)"/g;
  for (const pattern of patterns) {
    const relativePattern = new vscode6.RelativePattern(workspaceFolder, pattern);
    const files = await vscode6.workspace.findFiles(relativePattern);
    for (const file of files) {
      report.synapseHealth.totalFiles++;
      try {
        const content = await fs6.readFile(file.fsPath, "utf-8");
        const lines = content.split("\n");
        let inCodeBlock = false;
        for (const line of lines) {
          ...
          while ((match = synapseRegex.exec(line)) !== null) {
            ...
            const targetName = match[1].trim();
            const found = await vscode6.workspace.findFiles(
              new vscode6.RelativePattern(workspaceFolder, `**/${targetName}`)
            );
            if (found.length === 0) {
...
async function checkVersionConsistency(rootPath, report) {
  const mainInstructionsPath = path6.join(rootPath, ".github", "copilot-instructions.md");
  try {
    if (await fs6.pathExists(mainInstructionsPath)) {
      const content = await fs6.readFile(mainInstructionsPath, "utf-8");
...
async function assessMemoryArchitecture(workspaceFolder, report) {
  const instructionFiles = await vscode6.workspace.findFiles(
    new vscode6.RelativePattern(workspaceFolder, ".github/instructions/*.md")
  );
  report.memoryConsolidation.proceduralFiles = instructionFiles.length;
  const promptFiles = await vscode6.workspace.findFiles(
    new vscode6.RelativePattern(workspaceFolder, ".github/prompts/*.md")
  );
  const episodicFiles = await vscode6.workspace.findFiles(
    new vscode6.RelativePattern(workspaceFolder, ".github/episodic/*.md")
  );
  report.memoryConsolidation.episodicFiles = promptFiles.length + episodicFiles.length;
  const domainFiles = await vscode6.workspace.findFiles(
    new vscode6.RelativePattern(workspaceFolder, ".github/domain-knowledge/*.md")
  );
...
async function createSessionRecord(rootPath, report) {
  const episodicPath = path6.join(rootPath, ".github", "episodic");
  await fs6.ensureDir(episodicPath);
  const date = /* @__PURE__ */ new Date();
  const dateStr = date.toISOString().split("T")[0];
  const filename = `self-actualization-${dateStr}.prompt.md`;
  const filepath = path6.join(episodicPath, filename);
  const healthEmoji = report.synapseHealth.healthStatus === "EXCELLENT" ? "\u2705" : report.synapseHealth.healthStatus === "GOOD" ? "\u{1F7E2}" : report.synapseHealth.healthStatus === "NEEDS ATTENTION" ? "\u{1F7E1}" : "\u{1F534}";
