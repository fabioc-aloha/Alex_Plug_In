...
export const REPLICATE_MODELS: Record<string, ReplicateModel> = {
  ...
  ideogram: {
    ...
    label: "Ideogram v2",
    cost: "$0.08",
    bestFor: "Fast text rendering — signage, typography, badges",
    keywords: [
      "ideogram",
      "text in image",
      "image with text",
...
// ---------------------------------------------------------------------------
// Upscaling Model Catalog
// ---------------------------------------------------------------------------

export interface UpscalingModel {
...
/**
 * Upscaling models for image super-resolution.
 */
export const UPSCALING_MODELS: Record<string, UpscalingModel> = {
  realEsrgan: {
    id: "nightmareai/real-esrgan",
    label: "Real-ESRGAN",
    cost: "$0.002",
    bestFor: "General-purpose upscaling, photo enhancement",
    maxScale: 4,
  },
  swinIR: {
    ...
    label: "SwinIR",
    cost: "$0.005",
    bestFor: "Noise reduction, compression artifact removal",
    maxScale: 4,
  },
  codeformer: {
    ...
    label: "CodeFormer",
    cost: "$0.008",
    bestFor: "Face restoration and enhancement",
    maxScale: 2,
  },
  clarityUpscaler: {
    ...
    cost: "$0.10",
    bestFor: "Creative upscaling with AI enhancement",
    maxScale: 4,
  },
};
...
export function selectModelForPrompt(
  ...
  if (textKeywords.some((kw) => lower.includes(kw))) {
    return preferSpeed
      ? REPLICATE_MODELS.ideogramTurbo
      : REPLICATE_MODELS.ideogram;
  }

  // Speed / prototyping
  if (
    ["fast", "quick", "draft", "prototype", "test", "iterate", "cheap"].some(
  ...
  // High resolution
  if (
    ["2k", "3k", "cinematic", "high resolution", "large"].some((kw) =>
      lower.includes(kw),
    )
  ) {
  ...
  if (
    ["production", "professional", "photo", "photorealistic"].some((kw) =>
      lower.includes(kw),
    )
  ) {
  ...
  // Artistic / LoRA
  if (
    ["artistic", "art style", "lora", "fine-tune", "stable diffusion"].some(
      (kw) => lower.includes(kw),
    )
  ) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        try {
          resolve(
            JSON.parse(Buffer.concat(chunks).toString()) as ReplicatePrediction,
          );
        } catch (err) {
          reject(err);
        }
...
export interface UpscaleImageOptions {
  /** Scale factor (2x, 4x) */
  scale?: number;
  /** Whether to apply face enhancement (CodeFormer) */
  faceEnhance?: boolean;
}

export interface UpscalingQuickPickItem extends vscode.QuickPickItem {
  model: UpscalingModel;
}

/**
 * Upscale an image using super-resolution models.
 *
 * @param apiToken   Replicate API token
 * @param imageUrl   URL of the image to upscale (must be publicly accessible)
 * @param modelKey   Key from UPSCALING_MODELS
...
export async function upscaleImage(
  apiToken: string,
  imageUrl: string,
  modelKey: keyof typeof UPSCALING_MODELS,
  options: UpscaleImageOptions = {},
  progress?: vscode.Progress<{ message?: string }>,
): Promise<string | null> {
  const model = UPSCALING_MODELS[modelKey];
  if (!model) {
    throw new Error(`Unknown upscaling model: ${modelKey}`);
  }

  progress?.report({ message: `Upscaling with ${model.label}…` });
  ...
  if (modelKey === "realEsrgan") {
    input.scale = options.scale ?? 4;
    input.face_enhance = options.faceEnhance ?? false;
  } else if (modelKey === "swinIR") {
    input.scale = options.scale ?? 4;
  } else if (modelKey === "codeformer") {
    input.fidelity = 0.7; // Balance restoration vs. original features
    input.upscale = options.scale ?? 2;
  } else if (modelKey === "clarityUpscaler") {
    input.scale_factor = options.scale ?? 2;
    input.resemblance = 0.6;
    input.creativity = 0.35;
  }

  let prediction = await createPrediction(apiToken, model.id, input);
...
/**
 * Build Quick Pick items for upscaling model selection.
 */
export function buildUpscalingQuickPickItems(): UpscalingQuickPickItem[] {
  return Object.entries(UPSCALING_MODELS).map(([, model]) => ({
    label: `$(symbol-color) ${model.label}`,
    description: `${model.cost} — up to ${model.maxScale}x`,
