...
export const TOKEN_CONFIGS: Record<string, TokenConfig> = {
  GITHUB_TOKEN: {
    ...
    getUrl:
      "https://github.com/settings/tokens/new?description=Alex%20Global%20Knowledge&scopes=repo",
  ...
  REPLICATE_API_TOKEN: {
    ...
    description:
      "API token for AI image generation and model inference via Replicate",
 ...
 */
export async function initSecretsManager(
  ...
  for (const config of Object.values(TOKEN_CONFIGS)) {
    ...
    if (envValue && !storedValue) {
      try {
        ...
        logInfo(
          `[Alex][SecretsManager] Migrated ${config.envVar} to secure storage`,
        );
      } catch (error) {
        console.error(
          `[Alex][SecretsManager] Failed to migrate ${config.envVar}:`,
          error,
        );
      }
    }
...
export async function setToken(
  tokenName: keyof typeof TOKEN_CONFIGS,
  token: string,
): Promise<void> {
  if (!secretStorage) {
...
export async function clearAllTokens(): Promise<void> {
  ...
  for (const config of Object.values(TOKEN_CONFIGS)) {
    try {
      ...
      console.error(
        `[Alex][SecretsManager] Failed to clear ${config.displayName}:`,
        error,
      );
    }
  }
 ...
 */
export async function exportSecretsToEnv(
  targetFolder?: string,
): Promise<{ exported: number; filePath: string | null }> {
  const workspaceFolders = vscode.workspace.workspaceFolders;
  if (!workspaceFolders || workspaceFolders.length === 0) {
    vscode.window.showWarningMessage(
      "No workspace folder open. Cannot export secrets.",
    );
  ...
  if (exportedCount === 0) {
    vscode.window.showInformationMessage(
      "No secrets to export. Configure API keys first.",
    );
    return { exported: 0, filePath: null };
  }
  ...
  try {
    ...
    logInfo(
      `[Alex][SecretsManager] Exported ${exportedCount} secrets to ${envPath}`,
    );
...
export async function showTokenManagementPalette(): Promise<void> {
  ...
  if (selected.tokenName === "__HELP__") {
    ...
    const message = Object.entries(TOKEN_CONFIGS)
      .map(
        ([_, config]) =>
          `**${config.displayName}**: ${config.getUrl || "Contact administrator"}`,
      )
      .join("\n\n");
...
async function parseEnvFile(uri: vscode.Uri): Promise<DetectedSecret[]> {
  ...
  try {
    ...
    console.error(
      `[Alex][SecretsManager] Failed to parse ${uri.fsPath}:`,
      error,
    );
  }
...
export async function showEnvSecretsMigrationUI(): Promise<void> {
  ...
  const summary =
    `🔍 Found ${secrets.length} potential secret(s) in .env files:\n\n` +
    (recognized.length > 0
      ? `✅ ${recognized.length} recognized (can auto-migrate)\n`
      : "") +
    (unrecognized.length > 0
      ? `⚠️ ${unrecognized.length} custom (requires manual setup)\n`
      : "");
 ...
 */
async function autoMigrateRecognizedSecrets(
  ...
  for (const secret of secrets) {
    ...
    try {
      ...
      console.error(
        `[Alex][SecretsManager] Failed to migrate ${secret.key}:`,
        error,
      );
      skipped++;
    }
  ...
  const message =
    ...
    `Skipped: ${skipped}\n\n` +
    (migrated > 0
      ? `⚠️ Update your code to use VS Code SecretStorage instead of process.env`
      : "");
...
async function showSecretReviewUI(secrets: DetectedSecret[]): Promise<void> {
  ...
  const selected = await vscode.window.showQuickPick(items, {
    placeHolder:
      "Select secrets to migrate (recognized secrets auto-map to Alex tokens)",
