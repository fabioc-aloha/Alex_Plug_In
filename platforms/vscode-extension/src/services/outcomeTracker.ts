...
export async function recordPositiveOutcome(adviceId?: string): Promise<void> {
  ...
  vscode.window.showInformationMessage(
    "👍 Outcome recorded — Alex learns from this!",
    { modal: false },
  );
}
...
export async function recordNegativeOutcome(adviceId?: string): Promise<void> {
  ...
  vscode.window.showInformationMessage(
    "👎 Outcome recorded — Alex will adjust confidence in this area.",
    { modal: false },
  );
}
...
async function recordOutcome(
  ...
  if (!id) {
    vscode.window.showWarningMessage(
      "No recent advice to rate. Use @alex first, then rate the response.",
    );
    return;
  }
...
export function getConfidenceForDomain(domain: string): number {
  const records = loadRecords().filter(
    (r) => r.domain === domain && r.outcome !== null,
  );
  if (records.length < 3) {
    return 0.7;
  }
...
export function getDomainStats(): DomainStats[] {
  ...
  for (const [domain, domainRecords] of byDomain) {
    const positiveCount = domainRecords.filter(
      (r) => r.outcome === "positive",
    ).length;
    const negativeCount = domainRecords.filter(
      (r) => r.outcome === "negative",
    ).length;
...
export async function showOutcomeStatsCommand(): Promise<void> {
  ...
  for (const s of stats) {
    ...
    panel.appendLine(
      `  ${label} ${bar}  ${String(pct).padStart(3)}%  (${s.positiveCount}👍  ${s.negativeCount}👎  ${s.pendingCount} pending)`,
    );
  }
  panel.appendLine("");
  panel.appendLine(
    `  Total rated: ${stats.reduce((n, s) => n + s.positiveCount + s.negativeCount, 0)} advice records`,
  );
  panel.show();
}
