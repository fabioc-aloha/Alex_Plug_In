...
suite('Welcome View Test Suite', () => {
    ...
    suite('Welcome command routing & message guard', () => {
        ...
        test('ignores malformed messages safely', async () => {
            ...
            let called = false;
            (vscode.commands as any).executeCommand = async () => { called = true; };
        ...
        test('routes commandMap entries via executeCommandSafely (awaited)', async () => {
            ...
            (vscode.commands as any).executeCommand = async (...args: any[]) => { calls.push(args); };
            (vscode.window as any).withProgress = async (_opts: any, task: any) => task();
        ...
        test('persists tab switch in globalState memento', async () => {
            const ctx = makeFakeContext();
            const provider = new WelcomeViewProvider(ctx.extensionUri, ctx);
            await provider.handleMessageForTest({ command: 'tabSwitch', tabId: 'mind' });
        ...
        test('toggleSetting guards invalid keys and updates allowed keys', async () => {
            ...
            (vscode.workspace as any).getConfiguration = () => ({ update: async (...args: any[]) => { updateCalls.push(args); } });
            const warned: string[] = [];
            (vscode.window as any).showWarningMessage = (msg: string) => { warned.push(msg); return Promise.resolve(undefined); };

            // valid toggle
            await provider.handleMessageForTest({ command: 'toggleSetting', key: 'alex.voice.enabled', value: true });
            assert.strictEqual(updateCalls.length, 1);

            // invalid key should warn
            await provider.handleMessageForTest({ command: 'toggleSetting', key: 'alex.notAllowed', value: false });
        ...
        test('openDoc: commands resolve markdown preview with docUri', async () => {
            ...
            await provider.handleMessageForTest({ command: 'openDoc:COGNITIVE-ARCHITECTURE' });
