...
import { PERSONAS, Persona } from "./personaDefinitions";
import type {
      ...
      for (const indicator of indicators) {
        if (indicator.startsWith("*")) {
          ...
          if (entries.includes(dir)) {
            const statResult = await workspaceFs.stat(
...
export async function detectAndUpdateProjectPersona(
  ...
  assertDefined(
    workspaceRoot,
    "workspaceRoot is required for persona detection",
  );
