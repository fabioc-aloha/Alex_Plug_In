...
function inferCategory(
  title: string,
  summary: string,
  currentCategory: GlobalKnowledgeCategory,
): GlobalKnowledgeCategory {
...
function normalizeEntry(
  ...
  if (!entry.tags || entry.tags.length === 0) {
    ...
    if (generatedTags.length > 0) {
      ...
      details.push(
        `Generated tags for "${entry.title}": ${generatedTags.join(", ")}`,
      );
      modified = true;
    }
  }
  ...
  if (inferredCategory !== entry.category) {
    details.push(
      `Fixed category for "${entry.title}": ${entry.category} → ${inferredCategory}`,
    );
  ...
  if (normalizedSource !== entry.sourceProject) {
    details.push(
      `Normalized source for "${entry.title}": ${entry.sourceProject} → ${normalizedSource}`,
    );
