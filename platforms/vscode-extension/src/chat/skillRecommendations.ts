...
export async function trackRecommendationFeedback(
  ...
  try {
    ...
    // Get current count
    const currentCount = vscode.workspace
    ...
    try {
      await vscode.workspace
        .getConfiguration(context)
        .update(key, currentCount + 1, vscode.ConfigurationTarget.Global);
    } catch (globalError) {
      // Global config might be read-only (corporate policy) - try workspace
      await vscode.workspace
        .getConfiguration(context)
        .update(key, currentCount + 1, vscode.ConfigurationTarget.Workspace);
    }
  } catch (error) {
