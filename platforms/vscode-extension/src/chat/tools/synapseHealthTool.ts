...
export class SynapseHealthTool implements vscode.LanguageModelTool<ISynapseHealthParams> {
  ...
  async invoke(
    ...
    if (!workspaceFolders) {
      return new vscode.LanguageModelToolResult([
        new vscode.LanguageModelTextPart(
          "No workspace folder open. Cannot scan for synaptic connections.",
        ),
      ]);
    }
    ...
    const knownFileBasenames = new Set(
      allMdFiles.map((f) => path.basename(f.fsPath).toLowerCase()),
    );
        ...
        if (Array.isArray(sjData.connections)) {
          for (const conn of sjData.connections) {
            ...
            if (!(await workspaceFs.pathExists(targetFullPath))) {
              ...
              if (options.input.detailed) {
                issues.push(
                  `- ${path.basename(sjFile.fsPath)} → ${conn.target} (not found)`,
                );
              }
            }
          }
