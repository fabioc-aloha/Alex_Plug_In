...
export class MemorySearchTool implements vscode.LanguageModelTool<IMemorySearchParams> {
  ...
  async invoke(
    ...
    if (results.length === 0) {
      try {
        globalResults = await searchGlobalKnowledge(options.input.query, {
          limit: 5,
        });
      } catch {
    ...
    if (results.length === 0 && globalResults.length === 0) {
      return new vscode.LanguageModelToolResult([
        new vscode.LanguageModelTextPart(
          `No matches found for "${options.input.query}" in local ${memoryType} memory or global knowledge base.`,
        ),
      ]);
    }
