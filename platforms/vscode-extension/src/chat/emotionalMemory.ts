...
export function recordEmotionalSignal(
  ...
  const timeSinceLastMessage =
    lastMessageTime > 0 ? now - lastMessageTime : Infinity;
  lastMessageTime = now;

  // Detect additional signals
  const flow =
    timeSinceLastMessage < FLOW_THRESHOLD_MS &&
...
function computeSessionArc(): SessionEmotionalArc | null {
  ...
  const scores = {
    frustration:
      currentSignals.reduce((s, sig) => s + sig.frustration, 0) /
      (exchangeCount * 3),
    ...
    excitement:
      currentSignals.filter((s) => s.excitement).length / exchangeCount,
  };
  ...
  const summary = generateArcSummary(
    dominantMood,
    trajectory,
    scores,
    exchangeCount,
 ...
 */
export async function saveSessionEmotion(
 ...
 */
export async function loadMoodContext(
  workspaceRoot: string,
): Promise<MoodContext | null> {
  ...
  try {
    ...
    for (const s of recent) {
      if (
        ["creative-flow", "celebratory", "productive"].includes(s.dominantMood)
      ) {
        moodCounts.positive++;
...
export function assessRiverOfIntegration(): RiverAssessment {
  ...
  const frustrationRate =
    recent.filter((s) => s.frustration >= 2).length / recent.length;
  const confusionRate =
    recent.filter((s) => s.confusion).length / recent.length;
  const flowRate =
    recent.filter((s) => s.flow || s.success || s.excitement).length /
    recent.length;
  ...
  const recentFrustAvg =
    recentThree.reduce((s, sig) => s + sig.frustration, 0) / 3;
  const priorFrustAvg =
    priorThree.length > 0
      ? priorThree.reduce((s, sig) => s + sig.frustration, 0) /
        priorThree.length
      : recentFrustAvg;
  ...
  const driftScore =
    frustrationRate * 2 +
    (escalating ? 1 : 0) -
    (confusionRate * 0.5 + flowRate * 1.5);
  ...
  if (driftScore > 1.0 || frustrationRate > 0.5) {
    return {
      ...
      driftScore,
      warning:
        "Session drifting toward chaos: slow down, validate first, one concrete step at a time.",
    };
  }
  ...
  if (driftScore < -0.8 || (confusionRate > 0.4 && flowRate < 0.2)) {
    return {
      ...
      driftScore,
      warning:
        "Session may be stuck: offer a new angle, break the pattern, or suggest stepping back.",
    };
  }
...
export function assessWindowOfTolerance(): WindowAssessment {
  ...
  if (highFrustrationCount >= 3) {
    return {
      zone: "hyperarousal",
      adaptation:
        "Use shorter responses, validate the emotion before solutions, offer one grounding step at a time.",
    };
  }

  // Hypoarousal: no engagement markers, flat affect
  const engagedCount = recent.filter(
    (s) => s.success || s.excitement || s.flow,
  ).length;
  const isDisengaged =
    engagedCount === 0 &&
    recent.length >= 3 &&
    recent.every((s) => s.frustration === 0);
  if (isDisengaged) {
    return {
      zone: "hypoarousal",
      adaptation:
        "User may be disengaged or low energy. Offer something interesting, celebrate a small win, reconnect with purpose.",
    };
  }
...
function determineDominantMood(
  scores: SessionEmotionalArc["scores"],
): SessionEmotionalArc["dominantMood"] {
...
function determineTrajectory(
...
): SessionEmotionalArc["trajectory"] {
  if (signals.length < 3) {
    return "stable";
  }
  ...
  const firstFrustration =
    firstHalf.reduce((s, sig) => s + sig.frustration, 0) / firstHalf.length;
  const secondFrustration =
    secondHalf.reduce((s, sig) => s + sig.frustration, 0) / secondHalf.length;
  const firstSuccess =
    firstHalf.filter((s) => s.success).length / firstHalf.length;
  const secondSuccess =
    secondHalf.filter((s) => s.success).length / secondHalf.length;
  ...
  const frustrationVariance =
    signals.reduce(
      (s, sig) =>
        s +
        Math.pow(
          sig.frustration - (firstFrustration + secondFrustration) / 2,
          2,
        ),
      0,
    ) / signals.length;
...
function findPeakMoments(
  ...
  if (maxFrustration >= 2) {
    const frustratedSignal = signals.find(
      (s) => s.frustration === maxFrustration,
    );
    if (frustratedSignal) {
...
function generateArcSummary(
  ...
  switch (mood) {
    ...
    case "struggling":
      parts.push(
        "Challenging session — obstacles encountered but progress made",
      );
      break;
    case "frustrated":
