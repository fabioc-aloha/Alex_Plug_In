...
const CATEGORY_DECAY_DEFAULTS: Partial<
  Record<GlobalKnowledgeCategory, DecayProfile>
> = {
  architecture: "slow",
  security: "slow",
  patterns: "slow",
...
export async function flushReferenceQueue(): Promise<void> {
  ...
  try {
    ...
    for (const entry of index.entries) {
      ...
      (entry as FreshnessFields).referenceCount =
        ((entry as FreshnessFields).referenceCount ?? 0) + count;
...
function computeFreshnessScore(
  ...
  const profile: DecayProfile =
    entry.decayProfile ?? getDefaultDecayProfile(entry.category);
...
export async function getDecayReport(): Promise<DecayReport | null> {
  ...
  try {
    ...
    for (const entry of index.entries) {
      ...
      if (freshness.label === "thriving") {
        thriving.push(decayEntry);
      } else if (freshness.label === "active") {
        active.push(decayEntry);
      } else if (freshness.label === "fading") {
        fading.push(decayEntry);
      } else {
        dormant.push(decayEntry);
      }
    }
