...
const COGNITIVE_TASKS: Record<string, CognitiveTask> = {
  meditation: {
    ...
    warning:
      "For best results, switch to Opus or GPT-5.2. Consolidation quality may be limited on current model.",
  },
  selfActualization: {
    ...
    warning:
      "Self-actualization works best with Frontier models. Results may be shallow on current model.",
  },
  architectureRefactoring: {
    ...
    warning:
      "Complex architecture changes need deep context. Consider switching to a Frontier model.",
  },
  bootstrapLearning: {
    ...
    warning:
      "Skill acquisition needs maximum reasoning. A Frontier model will produce better results.",
  },
  skillSelection: {
    ...
    warning:
      "SSO works on Capable+ models. Current model may produce incomplete plans.",
  },
  dream: {
...
export function detectTaskFromPrompt(prompt: string): string | undefined {
  ...
  // Frontier tasks
  if (/meditat|consolidat|reflect/.test(promptLower)) {
    return "meditation";
  }
  if (/self-actuali|deep assess|architecture review/.test(promptLower)) {
    return "selfActualization";
  }
  if (/refactor.*architecture|restructure|major redesign/.test(promptLower)) {
    return "architectureRefactoring";
  }
  if (/learn.*new|bootstrap|acquire.*skill|teach me/.test(promptLower)) {
    return "bootstrapLearning";
  }

  // Capable tasks
  if (/dream|maintenance|health check|synapse/.test(promptLower)) {
    return "dream";
  }
  if (/review.*code|code review|pr review/.test(promptLower)) {
    return "codeReview";
  }
  if (/debug|fix.*bug|troubleshoot|error/.test(promptLower)) {
    return "debugging";
  }
  if (/skill select|plan.*task|optimize.*skill/.test(promptLower)) {
    return "skillSelection";
  }

  // Efficient tasks
  if (/format|rename|simple edit|typo|spelling/.test(promptLower)) {
    return "simpleEdits";
  }
  if (/doc|readme|comment|jsdoc/.test(promptLower)) {
    return "documentation";
  }
  if (/what is|explain|look up|find|search/.test(promptLower)) {
    return "lookup";
  }
...
export function getModelAdvice(
  ...
  const detectedTask =
    taskKey || (prompt ? detectTaskFromPrompt(prompt) : undefined);
  ...
  if (currentOrder < requiredOrder) {
    return {
      ...
      suggestedModels:
        TIER_MODEL_SUGGESTIONS[task.minimumTier as ModelTier] || [],
    };
  }

  // Check if downgrade possible (cost savings)
  if (
    currentOrder > requiredOrder &&
    currentModel.tier === "frontier" &&
    task.minimumTier === "efficient"
  ) {
    return {
      action: "downgrade",
 ...
 */
export function formatModelDashboard(
  model: DetectedModel,
  prompt?: string,
): string {
  const tierEmoji = {
...
export { MODEL_TIERS, COGNITIVE_TASKS, TIER_MODEL_SUGGESTIONS };
