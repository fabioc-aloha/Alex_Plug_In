...
import {
  loadPeripheralObservations,
  PeripheralObservations,
} from "./fileWatcher";
...
function getGitSummary(projectPath: string): GitSummary | undefined {
  ...
  const logOut = runGit(
    projectPath,
    'log --oneline --no-merges -5 --pretty=format:"%s"',
  );
  const lastCommits = logOut
    ? logOut
        ...
        .slice(0, 3)
        .map((msg) =>
          msg.replace(/\r$/, "").replace(/^"|"$/g, "").substring(0, 60),
        )
    : [];
...
function getShallowGitStatus(projectPath: string): {
  ...
  const uncommittedCount = statusOut
    ...
    : 0;
  const lastCommit =
    runGit(projectPath, 'log --oneline --no-merges -1 --pretty=format:"%s"')
      .replace(/^"|"$/g, "")
...
function detectManifests(projectPath: string): string[] {
  return MANIFEST_CHECKS.filter((check) =>
    fs.existsSync(path.join(projectPath, check.file)),
...
function detectTestFramework(projectPath: string): string | undefined {
  try {
    ...
    if (fs.existsSync(pkgPath)) {
      const pkg = fs.readJsonSync(pkgPath) as Record<
        string,
...
const _depCache = new Map<
  string,
  { result: DependencyFreshnessResult; expires: number }
>();
...
function classifySeverity(
  ...
  if (
    (parseInt(lParts[1] ?? "0", 10) || 0) >
    (parseInt(cParts[1] ?? "0", 10) || 0)
  ) {
    return "minor";
  }
  return "patch";
...
export function getDependencyFreshness(
  ...
  try {
    ...
    if (typeof spawnErr.stdout === "string") {
      ...
      _depCache.set(workspaceRoot, {
        result,
        expires: Date.now() + DEP_CACHE_TTL_MS,
      });
      return result;
    }
  }
  ...
  try {
    ...
    const result: DependencyFreshnessResult = {
      outdated: [],
      scannedAt: Date.now(),
      error: "Failed to parse npm outdated output",
    };
    _depCache.set(workspaceRoot, {
      result,
      expires: Date.now() + DEP_CACHE_TTL_MS,
...
export function getTestRunnerStatus(
  ...
  for (const candidate of candidates) {
    try {
      ...
      const lastRunPassed =
        raw.success !== undefined ? raw.success : failed === 0 && total > 0;
  ...
  const coverageSummary = path.join(
    workspaceRoot,
    "coverage",
    ...
    if (fs.existsSync(coverageSummary)) {
      ...
      try {
        const summary: JestCoverageSummary = JSON.parse(
          fs.readFileSync(coverageSummary, "utf-8"),
        );
...
function getRecentChanges(projectPath: string, maxFiles = 8): RecentChange[] {
  ...
  function walkDir(dir: string, depth: number) {
    if (depth > 4 || results.length >= maxFiles * 2) {
      return;
    }
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          if (ageMs < cutoffMs) {
            ...
            results.push({
              file: relPath,
              minsAgo: Math.round(ageMs / 60_000),
            });
          }
        } catch {
  ...
  return results.sort((a, b) => a.minsAgo - b.minsAgo).slice(0, maxFiles);
}
...
async function discoverPeerProjects(
  workspaceRoot: string,
): Promise<PeerProject[]> {
  ...
  for (const entry of entries) {
    if (peers.length >= MAX_PEER_PROJECTS) {
      break;
    }
    if (!entry.isDirectory()) {
      continue;
 ...
 */
export async function gatherPeripheralContext(
  workspaceRoot: string,
): Promise<PeripheralContext> {
  if (!workspaceRoot) {
    return {
      parentFolder: "",
      peerProjects: [],
  ...
  const [peerProjects, recentChanges, fileWatcherObservations] =
    await Promise.all([
  ...
  const dependencyFreshness = hasPackageJson
    ? getDependencyFreshness(workspaceRoot)
    : undefined;
