    ...
    async (uri?: vscode.Uri) => {
      ...
      if (!text) {
        const insight = await vscode.window.showInputBox({
          ...
          placeHolder:
            "e.g., useEffect cleanup runs before next effect - helped fix my memory leak",
          ignoreFocusOut: true,
        });
...
`;
    ...
    async (uri?: vscode.Uri) => {
      ...
      if (!text) {
        const query = await vscode.window.showInputBox({
          ...
          placeHolder:
            "e.g., error handling patterns, React hooks, TypeScript generics",
          ignoreFocusOut: true,
        });
      ...
      await vscode.window.withProgress(
        ...
        async () => {
          ...
          const items: KnowledgeQuickPickItem[] = results.map((r) => ({
            label: r.entry.title,
            description:
              r.entry.type === "pattern" ? "📁 Pattern" : "💡 Insight",
    ...
    async () => {
      ...
      const updateItems = async (query: string) => {
        ...
        if (!query) {
          ...
          quickPick.items = [
            {
              label: "$(folder) Patterns",
              description: `${status.totalPatterns} patterns`,
              alwaysShow: true,
            },
            {
              label: "$(lightbulb) Insights",
              description: `${status.totalInsights} insights`,
              alwaysShow: true,
            },
            {
              label: "$(pulse) Run Health Check",
              description: "Check architecture status",
              alwaysShow: true,
            },
          ];
        } else {
  ...
  // Image commands registered in contextMenuImage.ts
