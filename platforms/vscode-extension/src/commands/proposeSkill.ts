...
async function packageAndPresentSkill(
  ...
  if (action === "Open Package") {
    ...
    vscode.commands.executeCommand(
      "vscode.openFolder",
      vscode.Uri.file(GLOBAL_KNOWLEDGE_PATH),
      true,
    );
  }
    ...
    await vscode.window.withProgress(
      ...
      async (progress, token) => {
        ...
        outputChannel.appendLine(
          `📁 Found ${heirSkills.length} skills in project`,
        );

        if (heirSkills.length === 0) {
          vscode.window.showInformationMessage(
            "No skills found in this project to propose.",
          );
          return;
        }
        ...
        progress.report({
          message: "Filtering proposable skills...",
          increment: 20,
        });
        const proposableSkills = heirSkills.filter(
          (skill) => !skill.inheritedFromGK,
        );
        ...
        outputChannel.appendLine(
          `✅ ${proposableSkills.length} skills available for proposal`,
        );

        const result = await selectAndValidateSkill(
          projectSkillsPath,
          proposableSkills,
          outputChannel,
          progress,
...
function parseSkillMetadata(content: string): SkillMetadata {
  ...
  if (yamlMatch) {
    ...
    if (applyToMatch) {
      metadata.applyTo = applyToMatch[1].trim();
    }
...
function detectCategory(content: string, _metadata: SkillMetadata): string {
  ...
  if (contentLower.includes("deploy") || contentLower.includes("deployment")) {
    return "deployment";
  }
  if (
  ...
  if (contentLower.includes("debug") || contentLower.includes("troubleshoot")) {
    return "debugging";
  }
  if (
  ...
  if (
    ...
    contentLower.includes("design pattern")
  ) {
    return "architecture";
  }
...
function calculateValidationScore(
  ...
  const hasGenericTerms = !content.match(
    /\b(my-app|our-project|this-company)\b/i,
  );
  if (hasGenericTerms) {
    score += 2;
  }
...
async function validateSkill(
  ...
  const projectSpecific = skill.content.match(
    /\b(TODO|FIXME|my-app|our-project|this-company)\b/gi,
  );
  if (projectSpecific && projectSpecific.length > 2) {
    warnings.push(
      `Found ${projectSpecific.length} project-specific terms. Consider generalizing.`,
    );
  }
  ...
  if (skill.validationScore < 8) {
    warnings.push(
      `Promotion score is ${skill.validationScore}/12. Recommended minimum: 8. Consider adding missing sections.`,
    );
  }
  ...
  if (!skill.content.includes("Synapse")) {
    warnings.push(
      "No synapses found. Consider adding connections to related skills.",
    );
  }
 ...
 */
async function collectProposalMetadata(
  skill: HeirSkill,
  workspacePath: string,
): Promise<GKYamlFrontmatter | null> {
...
function detectTags(content: string, _metadata: SkillMetadata): string[] {
  ...
  if (contentLower.includes("node")) {
    tags.add("nodejs");
  }
  if (contentLower.includes("docker")) {
    tags.add("docker");
  }
  if (contentLower.includes("kubernetes")) {
    tags.add("kubernetes");
  }
  if (contentLower.includes("azure")) {
    tags.add("azure");
  }
  if (contentLower.includes("github")) {
    tags.add("github");
  }
  if (contentLower.includes("ci/cd")) {
    tags.add("ci-cd");
  }
  ...
  if (contentLower.includes("deployment")) {
    tags.add("deployment");
  }
  if (contentLower.includes("security")) {
    tags.add("security");
  }
  if (contentLower.includes("automation")) {
    tags.add("automation");
  }
