    ...
    await vscode.window.withProgress(
      ...
      async (progress, token) => {
        ...
        outputChannel.appendLine(
          `📚 Loaded ${registry.skills.length} skills from Global Knowledge`,
        );
        ...
        progress.report({
          message: "Scanning project skills...",
          increment: 20,
        });
        ...
        outputChannel.appendLine(
          `📁 Found ${existingSkills.size} existing skills in project`,
        );

        // Filter available skills
        const availableSkills = registry.skills.filter(
          (skill) =>
            skill.inheritance === "inheritable" &&
            !existingSkills.has(skill.id),
        );

        if (availableSkills.length === 0) {
          vscode.window.showInformationMessage(
            "All inheritable skills are already in this project.",
          );
          return;
        }

        outputChannel.appendLine(
          `✅ ${availableSkills.length} skills available for inheritance`,
        );
        ...
        progress.report({
          message: `Inheriting ${selected.length} skill(s)...`,
          increment: 20,
        });
        ...
        for (const item of selected) {
          ...
          try {
            ...
            failCount++;
            outputChannel.appendLine(
              `❌ Failed to inherit ${skill.name}: ${error}`,
            );
          }
        }
