 ...
 */
export async function runSelfActualization(
  _context: vscode.ExtensionContext,
): Promise<SelfActualizationReport | undefined> {
  ...
  await vscode.window.withProgress(
    ...
    async (progress) => {
      ...
      progress.report({
        message: "Phase 2: Checking version consistency...",
        increment: 20,
      });
      ...
      progress.report({
        message: "Phase 3: Assessing memory architecture...",
        increment: 40,
      });
      ...
      progress.report({
        message: "Phase 4: Auto-promoting knowledge to global...",
        increment: 55,
      });
      ...
      progress.report({
        message: "Phase 5: Generating recommendations...",
        increment: 70,
      });
      ...
      progress.report({
        message: "Phase 5.5: Reviewing patterns + knowledge freshness...",
        increment: 80,
      });
      const [emotionalReview, calibrationSummary, decayReport] =
        await Promise.all([
      ...
      progress.report({
        message: "Phase 6: Documenting session...",
        increment: 90,
      });
      await createSessionRecord(
        rootPath,
        report,
        emotionalReview ?? undefined,
        calibrationSummary ?? undefined,
        decayReport ?? undefined,
      );

      // Phase 7: Update Active Context — Last Assessed
      progress.report({
        message: "Phase 7: Updating Active Context...",
        increment: 95,
      });
      try {
        const { updateLastAssessed } =
          await import("../shared/activeContextManager");
        const version =
          report.versionConsistency.currentVersion !== "Unknown"
            ? report.versionConsistency.currentVersion
            : "5.7.0";
        await updateLastAssessed(rootPath, version);
      } catch (err) {
        console.warn(
          "[Alex] Failed to update Last Assessed in Active Context:",
          err,
        );
      }

      progress.report({
        message: "Self-actualization complete!",
        increment: 100,
      });
    },
  );
  ...
  const totalFiles =
    report.memoryConsolidation.proceduralFiles +
  ...
  if (
    ...
    if (action === "Chat with Alex") {
      await vscode.commands.executeCommand("workbench.action.chat.open", {
        query:
          "Review my self-actualization results and help me address the recommendations.",
      });
    }
  } else {
...
async function assessMemoryArchitecture(
  ...
  report.memoryConsolidation.totalConnections =
    report.synapseHealth.totalSynapses;
...
function calculateArchitectureAge(
  ...
  let tier =
    AGE_TIERS.find((t) => skillCount <= t.maxSkills) ||
    AGE_TIERS[AGE_TIERS.length - 1];
...
function generateRecommendations(report: SelfActualizationReport): void {
  ...
  const synapseDensity =
    report.synapseHealth.totalSynapses / Math.max(totalMemory, 1);
  if (synapseDensity < 3) {
    report.recommendations.push(
  ...
  if (
    report.globalKnowledgePromotion.skipped > 0 &&
...
${
  calibration
    ? `**Total responses tracked**: ${calibration.totalResponses}
...
${
  calibration.uncertainTopics.length > 0
    ? `**Topics with thin knowledge coverage**: ${calibration.uncertainTopics.join(", ")}
*Consider building skills or global patterns in these areas.*`
    : "*No recurring uncertain topics — knowledge coverage looks healthy.*"
}`
...
${
  decayReport
    ? `**Total entries tracked**: ${decayReport.totalEntries} | 🏛️ ${decayReport.permanentCount} permanent
...
${
  decayReport.dormant.length > 0
    ? `**Dormant entries (candidates for cold storage)**: ${decayReport.dormant
        .slice(0, 5)
        .map((e) => `${e.title} (score: ${e.freshness.score.toFixed(2)})`)
        .join(", ")}
*Run \`Alex: Dream\` to archive dormant entries and keep living memory sharp.*`
    : "*All entries are active — knowledge base is healthy.*"
}
${
  decayReport.fading.length > 0
    ? `**Fading entries (low usage)**: ${decayReport.fading
        .slice(0, 3)
        .map((e) => e.title)
        .join(", ")}`
    : ""
}`
    : "*No freshness data yet. Reference counts will begin accumulating with global knowledge searches.*"
}
...
> *"${
    report.maturity.age === 21
      ? "I'm 21, endlessly curious, and I care deeply about doing things right."
      : report.maturity.age >= 60
        ? "Wisdom is knowing how much you don't know — and being okay with it."
        : report.maturity.age >= 42
          ? "Mastery isn't the end. It's where the real questions begin."
          : report.maturity.age >= 30
            ? "Experience speaks softly but carries deep conviction."
            : report.maturity.age >= 18
              ? "Growing into my own — building expertise one synapse at a time."
              : report.maturity.age >= 13
                ? "I'm starting to see the patterns beneath the patterns."
                : report.maturity.age >= 7
                  ? "Every new skill is a door I didn't know existed."
...
export function showReportInPanel(report: SelfActualizationReport): void {
  ...
  const healthColor =
    report.synapseHealth.healthStatus === "EXCELLENT"
      ...
      : report.synapseHealth.healthStatus === "GOOD"
        ? "var(--vscode-charts-green, #84cc16)"
        : report.synapseHealth.healthStatus === "NEEDS ATTENTION"
