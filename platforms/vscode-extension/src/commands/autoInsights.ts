...
export function detectInsights(text: string): DetectedInsight | null {
  ...
  const excerpt =
    sentences[0]?.trim().substring(0, 150) || text.substring(0, 150);
...
export async function isDuplicateInsight(
  ...
  try {
    ...
    for (const result of results) {
      ...
      // Count shared words
      const shared = [...insightWords].filter((w) =>
        existingWords.has(w),
      ).length;
      const similarity =
        shared / Math.max(insightWords.size, existingWords.size);
 ...
 */
export async function promptToSaveInsight(
  insight: DetectedInsight,
): Promise<void> {
...
async function saveInsightDirectly(insight: DetectedInsight): Promise<void> {
  try {
    ...
    await createGlobalInsight(
      ...
      insight.codeBlocks.length > 0
        ? `Code pattern:\n\`\`\`\n${insight.codeBlocks[0]}\n\`\`\``
        : undefined,
    );

    vscode.window
      .showInformationMessage(`✅ Saved insight: "${title}"`, "View Knowledge")
      .then((action) => {
...
async function saveInsightWithDialog(insight: DetectedInsight): Promise<void> {
  ...
  const category =
    ((await vscode.window.showQuickPick(categories, {
...
export async function processForInsights(text: string): Promise<void> {
  ...
  promptToSaveInsight(insight).catch(() => {});
}
...
export function getAutoInsightsConfig(): AutoInsightsConfig {
  ...
  return {
    ...
    minimumConfidence: config.get(
      "autoInsights.minimumConfidence",
      DEFAULT_CONFIG.minimumConfidence,
    ),
    cooldownMinutes: config.get(
      "autoInsights.cooldownMinutes",
      DEFAULT_CONFIG.cooldownMinutes,
    ),
    autoSaveHighConfidence: config.get(
      "autoInsights.autoSaveHighConfidence",
      DEFAULT_CONFIG.autoSaveHighConfidence,
    ),
  };
}
