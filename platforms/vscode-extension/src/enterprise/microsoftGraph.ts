...
// ===========================================================================
// Email Sending Module (FishbowlGovernance heir pattern)
// ===========================================================================
...
/**
 * Email message for Graph sendMail
 */
...
/**
 * Send an email via Graph Mail API
 * Requires: Mail.Send scope
 */
export async function sendMail(message: EmailMessage): Promise<boolean> {
  const payload = {
    message: {
      ...
      toRecipients: message.toRecipients.map((addr) => ({
        emailAddress: { address: addr },
      })),
      ...(message.ccRecipients && {
        ccRecipients: message.ccRecipients.map((addr) => ({
          emailAddress: { address: addr },
        })),
      }),
      ...(message.attachments && {
...
/**
 * Helper to convert file content to base64 attachment
 */
export function createAttachment(
  ...
  return { name, contentBytes: base64, contentType };
}
