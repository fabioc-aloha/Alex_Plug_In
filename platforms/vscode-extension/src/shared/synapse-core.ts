...
export const consolidatedMappings: { [key: string]: string } = {
  ...
  "unified-consciousness.instructions.md":
    "alex-identity-integration.instructions.md",
  "dream-protocol-integration.prompt.md":
    "dream-state-automation.instructions.md",
  "dream-protocol-mastery-meditation.prompt.md":
    "dream-state-automation.instructions.md",
};
  ...
  try {
    ...
    for (const [name, fileType] of entries) {
      ...
      if (fileType === vscode.FileType.Directory) {
        ...
        const subResults = await findMdFilesRecursive(
          fullPath,
          rootPath,
          maxDepth - 1,
        );
...
|-------|------------|--------|--------|
${validation.checks
  .map((c) => `| ${c.name} | ${c.documentedCount} | ${c.actualCount} | ✅ |`)
  .join("\n")}
...
function generateInheritanceSection(lineage: InheritanceLineage): string {
  ...
  if (
    lineage.inheritedSkills.length === 0 &&
    lineage.versionDriftWarnings.length === 0
  ) {
    return `
...
|-------|--------|---------|-----------|
${lineage.inheritedSkills
  .map((s) => {
    const date =
      s.inheritedAt !== "unknown"
...
export function generateReportMarkdown(report: DreamReport): string {
  const isHealthy =
    report.brokenSynapses.length === 0 &&
    (report.docCountValidation?.isValid ?? true);
  const healthPercent =
    report.totalSynapses > 0
      ? Math.round(
          ((report.totalSynapses - report.brokenSynapses.length) /
            report.totalSynapses) *
            100,
        )
      : 100;
  ...
  const docCountSection = report.docCountValidation
    ? generateDocCountSection(report.docCountValidation)
    : "";

  // Generate inheritance lineage section
  const inheritanceSection = report.inheritanceLineage
    ? generateInheritanceSection(report.inheritanceLineage)
    : "";
...
${
  report.brokenSynapses.length > 0
    ? `- [ ] Review and repair ${report.brokenSynapses.length} broken synapse${report.brokenSynapses.length > 1 ? "s" : ""}
...
export async function saveReport(
  ...
  const reportPath = path.join(
    ...
    ".github",
    "episodic",
    `dream-report-${dateStr}-${Date.now()}.md`,
  );
