...
const GR = '\x1b[90m', M = '\x1b[35m', X = '\x1b[0m';

const issues = [];
const warnings = [];
const fixed = [];

function writePhase(n, name) { if (!quiet) console.log(`  ${C}[${n}] ${name}${X}`); }
function pass(msg) { if (detail && !quiet) console.log(`  ${G}[OK] ${msg}${X}`); }
function warn(msg) { if (detail && !quiet) console.log(`  ${Y}[WARN] ${msg}${X}`); warnings.push(msg); }
function fail(msg) { if (!quiet) console.log(`  ${R}[ERROR] ${msg}${X}`); issues.push(msg); }
...
let runPhases;
switch (mode) {
  case 'quick':  runPhases = quickPhases; break;
  case 'sync':   runPhases = syncPhases; break;
  case 'schema': runPhases = schemaPhases; break;
  case 'llm':    runPhases = llmPhases; break;
  default:       runPhases = Array.from({length: 35}, (_, i) => i + 1);
}
if (phaseFilter) runPhases = phaseFilter;

if (!quiet) console.log(`${GR}Brain QA — Mode: ${mode} | Phases: ${runPhases.join(',')} | Use --detail for verbose output${X}`);
...
function grepFiles(dir, nameRx, contentRx) {
  return findRecursive(dir, n => nameRx.test(n))
    .filter(f => contentRx.test(fs.readFileSync(f, 'utf8')));
}
...
if (needsSync) {
  if (!quiet) console.log(`  ${M}[Pre-Sync] Master -> Heir Synchronization${X}`);

  const syncScript = path.join(ghPath, 'muscles', 'sync-architecture.cjs');
  if (fs.existsSync(syncScript)) {
    try {
      execSync(`node "${syncScript}"`, { cwd: rootPath, stdio: detail ? 'inherit' : 'pipe' });
  ...
  if (fs.existsSync(heirSkillsDir)) {
    ...
    const masterOnlySkills = dirs(path.join(ghPath, 'skills'))
      .filter(s => !fs.existsSync(path.join(heirSkillsDir, s)));
      ...
      if (fs.existsSync(masterSyn) && fs.existsSync(heirSyn)) {
        if (fileHash(masterSyn) !== fileHash(heirSyn)) {
          ...
          if (mj && mj.connections) {
            mj.connections = mj.connections.filter(c =>
              !masterOnlySkills.some(mo => c.target && c.target.includes(mo))
            );
    ...
    if (synapseSyncCount > 0) pass(`Synced ${synapseSyncCount} synapse file(s)`);
...
if (runPhases.includes(1)) {
  ...
  for (const f of findRecursive(ghPath, n => n === 'synapses.json')) {
    ...
    for (const conn of j.connections) {
      const t = conn.target;
      if (!t || /^(external:|global-knowledge:\/\/|https?:\/\/|mailto:)/.test(t)) continue;
      let fullPath;
      if (/^\.github\/|^[A-Z].*\.md$/.test(t)) fullPath = path.join(rootPath, t);
...
if (runPhases.includes(2)) {
  ...
  if (stale.length === 0) pass('No skills have stale inheritance field (centralized in sync-architecture.cjs)');
  else fail(`Stale inheritance field in synapses.json (should be removed): ${stale.join(', ')}`);
}
...
if (runPhases.includes(3)) {
  ...
  const indexPath = path.join(ghPath, 'skills', 'memory-activation', 'SKILL.md');
  if (fs.existsSync(indexPath)) {
    const indexContent = fs.readFileSync(indexPath, 'utf8');
    const notIndexed = skillDirs.filter(s => s !== 'memory-activation' && !indexContent.includes(`${s} |`));
...
if (runPhases.includes(4)) {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    const overlaps = Object.entries(triggers).filter(([, v]) => v.includes(','));
...
if (runPhases.includes(5)) {
  ...
  if (fs.existsSync(heirSkillsDir)) {
    ...
    if (missingInHeir.length > 0) warn(`Missing in heir: ${missingInHeir.join(', ')}`);
    if (extraInHeir.length > 0) warn(`Extra in heir: ${extraInHeir.join(', ')}`);
    if (missingInHeir.length === 0 && extraInHeir.length === 0) pass('Skill directories match');
  } else {
...
if (runPhases.includes(6)) {
  ...
  for (const skill of dirs(path.join(ghPath, 'skills'))) {
    ...
    if (/"strength":\s*"(High|Medium|Critical|Low)"/.test(content)) critical.add(skill);
    if (!content.includes('"$schema"')) critical.add(skill);
    if (/"synapses":\s*\[/.test(content) && !/"connections":\s*\[/.test(content)) info.push(skill);
  }
  if (critical.size === 0) pass('Schema validation passed');
  else fail(`Schema issues: ${[...critical].join(', ')}`);
  if (info.length > 0) warn(`Legacy array name (works but prefer 'connections'): ${info.join(', ')}`);
}
...
if (runPhases.includes(8)) {
  ...
  const masterIdx = path.join(ghPath, 'skills', 'memory-activation', 'SKILL.md');
  const heirIdx = path.join(heirBase, '.github', 'skills', 'memory-activation', 'SKILL.md');
...
if (runPhases.includes(9)) {
  ...
  const catPath = path.join(rootPath, 'alex_docs', 'skills', 'SKILLS-CATALOG.md');
  if (fs.existsSync(catPath)) {
    ...
    if (m) {
      ...
      else fail(`Count mismatch: Catalog says ${catCount}, actual is ${actual}`);
    } else {
...
if (runPhases.includes(10)) {
  ...
  for (const f of coreFiles) {
    ...
    if (lineCount > 500) {
      budgetWarnings.push(`${id} is ${lineCount} lines (${charCount} chars) - consider trimming (auto-loaded every session)`);
    }
    if (/[┌┐└┘├┤┬┴┼│─═║╔╗╚╝╠╣╦╩╬]/.test(content)) {
      budgetWarnings.push(`${id} contains ASCII box-drawing art (use Mermaid or tables instead)`);
    }
  }
...
if (runPhases.includes(11)) {
  ...
  for (const skill of dirs(path.join(ghPath, 'skills'))) {
    ...
    if (fs.existsSync(skillMd)) {
      ...
      if (/description:\s*"Skill for alex .+ skill"/.test(content)) boilerplate.push(skill);
    }
  }
...
if (runPhases.includes(12)) {
  ...
  if (fs.existsSync(heirGh)) {
    ...
    if (fs.existsSync(profilePath)) {
      ...
      if (prof && prof.name && prof.name !== '') resetIssues.push('user-profile.json has non-empty name (PII leak)');
      if (prof && prof.nickname && prof.nickname !== '') resetIssues.push('user-profile.json has non-empty nickname (PII leak)');
    }
    if (!fs.existsSync(path.join(heirGh, 'config', 'user-profile.template.json'))) {
      resetIssues.push('user-profile.template.json missing (needed for runtime profile creation)');
    }

    const copilotPath = path.join(heirGh, 'copilot-instructions.md');
    if (fs.existsSync(copilotPath)) {
      const copilot = fs.readFileSync(copilotPath, 'utf8');
      if (!copilot.includes('## Identity')) resetIssues.push('Missing Identity section (v3 format)');
      if (!copilot.includes('## Active Context')) resetIssues.push('Missing Active Context section');
      if (/Focus Trifectas:\s*master-heir-management/.test(copilot)) resetIssues.push('Focus Trifectas has master-only values');
      if (/Last Assessed:\s*\d{4}-/.test(copilot)) resetIssues.push('Last Assessed should be "never" in heir');
      if (/Fabio|Correa|Calefato|Cardoso/.test(copilot)) resetIssues.push('Found hardcoded names in copilot-instructions.md');
    }
...
if (runPhases.includes(13)) {
  ...
  if (fs.existsSync(heirInstrDir)) {
    const mi = files(path.join(ghPath, 'instructions'), '.md').filter(f => !moInstr.includes(f)).sort();
    const hi = files(heirInstrDir, '.md').sort();
    const mp = files(path.join(ghPath, 'prompts'), '.md').filter(f => !moPrompts.includes(f)).sort();
...
if (runPhases.includes(14)) {
  ...
  if (agents.length === 0) agentIssues.push('No agents found');
    ...
    for (const a of agents) {
      ...
      if (!content.startsWith('---')) agentIssues.push(`${a}: Missing YAML frontmatter`);
      if (!content.includes('name:')) agentIssues.push(`${a}: Missing name field`);
    }
  }
  ...
  if (fs.existsSync(heirAgentsDir)) {
    ...
    const ha = fs.readdirSync(heirAgentsDir).filter(f => f.endsWith('.md')).sort();
    if (ma.length !== ha.length) agentIssues.push(`Agent count mismatch: Master=${ma.length}, Heir=${ha.length}`);
  }
...
if (runPhases.includes(15)) {
  ...
  const required = ['user-profile.template.json', 'cognitive-config-template.json'];
  const masterOnly = ['MASTER-ALEX-PROTECTED.json', 'cognitive-config.json', 'user-profile.json'];
  const configIssues = [];
  ...
  for (const cfg of required) {
    ...
    else if (cfg.endsWith('.json') && !readJSON(p)) configIssues.push(`Invalid JSON: ${cfg}`);
  }
  for (const cfg of masterOnly) {
    if (fs.existsSync(path.join(heirCfg, cfg))) configIssues.push(`Master-only file leaked: ${cfg}`);
  }
...
if (runPhases.includes(16)) {
  ...
  for (const skill of dirs(path.join(ghPath, 'skills'))) {
    ...
    if (!/^---\s*\n/.test(content)) {
      ...
      if (fmMatch) {
        const fm = fmMatch[1];
        if (!/name:\s*['"]?[\w-]+/.test(fm)) fmIssues.push(`${skill} - Missing 'name' in frontmatter`);
        if (!/description:\s*['"]?.+/.test(fm)) fmIssues.push(`${skill} - Missing 'description' in frontmatter`);
      }
    }
  }
...
if (runPhases.includes(17)) {
  ...
  for (const skill of internalSkills) {
    ...
    if (fs.existsSync(skillMd)) {
      const content = fs.readFileSync(skillMd, 'utf8');
      if (!/user-invokable:\s*false/.test(content)) viIssues.push(`${skill} should have user-invokable: false`);
    }
  }
...
if (runPhases.includes(18)) {
  ...
  for (const a of agents) {
    ...
    } else if (a !== 'alex.agent.md' && content.includes('handoffs:') && !/agent:\s*Alex/.test(content)) {
      handoffIssues.push(`${a} - No return-to-Alex handoff`);
    }
    if (content.includes('handoffs:')) {
      if (!content.includes('label:')) handoffIssues.push(`${a} - Handoff missing 'label'`);
      if (!content.includes('agent:')) handoffIssues.push(`${a} - Handoff missing 'agent'`);
    }
  }
...
if (runPhases.includes(20)) {
  ...
  for (const f of patterns) {
    ...
    if (/[┌┐└┘├┤┬┴┼│─═║╔╗╚╝╠╣╦╩╬]/.test(content)) {
      formatWarnings.push(`${name} - Contains box-drawing ASCII art (Mermaid or tables preferred)`);
    }
    const arrowLines = content.split('\n').filter(l => /^\s*[│↓↑<\->]/.test(l));
    if (arrowLines.length > 5) {
      formatWarnings.push(`${name} - Heavy use of ASCII arrows (structured format preferred)`);
    }
  }
...
if (runPhases.includes(21)) {
  ...
  if (fs.existsSync(agentsDir)) {
    for (const a of fs.readdirSync(agentsDir).filter(f => f.endsWith('.md'))) {
      const content = fs.readFileSync(path.join(agentsDir, a), 'utf8');
      for (const e of ['[OK]', '[ERROR]', '[WARN]']) {
        emojiCount += (content.match(new RegExp(e.replace(/[[\]]/g, '\\$&'), 'g')) || []).length;
      }
    }
  }
...
if (runPhases.includes(22)) {
  ...
  if (fs.existsSync(epDir)) {
    ...
    const dreams = allEp.filter(f => f.startsWith('dream-report-') || f.startsWith('dream-'));
    const meds = allEp.filter(f => f.startsWith('meditation-'));
    const selfAct = allEp.filter(f => f.startsWith('self-actualization-'));
    pass(`Episodic files: ${allEp.length} total (${dreams.length} dreams, ${meds.length} meditations, ${selfAct.length} self-actualizations)`);

    const legacyPrompts = allEp.filter(f => f.endsWith('.prompt.md'));
    if (legacyPrompts.length > 0) {
      fail(`Episodic has ${legacyPrompts.length} legacy .prompt.md files - archive to archive/upgrades/`);
    }

    const undated = allEp.filter(f => !/\d{4}-\d{2}-\d{2}/.test(f) && f !== '.markdownlint.json');
    if (undated.length > 0) warn(`Episodic has ${undated.length} undated files`);

    const invalidNames = allEp.filter(f =>
      f !== '.markdownlint.json' &&
      !/^(dream-report-|dream-|meditation-|self-actualization-|session-|chronicle-)/.test(f)
    );
    if (invalidNames.length > 0) {
      fail(`Episodic has ${invalidNames.length} files with non-standard naming`);
    }
  } else {
...
if (runPhases.includes(24)) {
  ...
  if (fs.existsSync(itDir)) {
    for (const expected of ['bug_report.md', 'feature_request.md']) {
      if (!fs.existsSync(path.join(itDir, expected))) warn(`Missing issue template: ${expected}`);
    }
    pass(`Issue templates present (${fs.readdirSync(itDir).filter(f => f.endsWith('.md')).length} templates)`);
  } else {
    warn('ISSUE_TEMPLATE/ not found');
  }
  if (fs.existsSync(path.join(ghPath, 'pull_request_template.md'))) pass('PR template present');
...
if (runPhases.includes(25)) {
  ...
  const missingRoot = rootFiles.filter(f => !fs.existsSync(path.join(ghPath, f)));
  if (missingRoot.length > 0) missingRoot.forEach(m => fail(`Missing .github/ root file: ${m}`));
  else pass(`All .github/ root files present (${rootFiles.length} files)`);

  const expectedDirs = ['agents', 'config', 'episodic', 'instructions', 'muscles', 'prompts', 'skills', 'assets', 'ISSUE_TEMPLATE'];
  const missingDirs = expectedDirs.filter(d => !fs.existsSync(path.join(ghPath, d)));
  if (missingDirs.length > 0) missingDirs.forEach(m => warn(`Missing .github/ subfolder: ${m}`));
...
if (runPhases.includes(26)) {
  ...
  if (fs.existsSync(docsPath)) {
    ...
    const expected = ['TRIFECTA-CATALOG.md', 'COGNITIVE-ARCHITECTURE.md', 'MEMORY-SYSTEMS.md', 'NEUROANATOMICAL-MAPPING.md', 'AGENT-CATALOG.md'];
    const missing = expected.filter(d => !fs.existsSync(path.join(archPath, d)));
    if (missing.length > 0) missing.forEach(m => fail(`Missing architecture doc: ${m}`));

    if (fs.existsSync(archPath)) {
      const staleVersions = [];
      for (const f of fs.readdirSync(archPath).filter(f => f.endsWith('.md'))) {
        const content = fs.readFileSync(path.join(archPath, f), 'utf8');
        if (/Alex v[23]\.\d+\.\d+/.test(content)) staleVersions.push(`${f} references pre-v5 Alex version`);
      }
    ...
    if (fs.existsSync(catPath)) {
      const content = fs.readFileSync(catPath, 'utf8');
      const masterCount = findRecursive(path.join(ghPath, 'skills'), n => n === 'SKILL.md').length;
      const m = content.match(/(?:Skill Count[:\s]+|Total[^\n]*?)(\d+)\s*skills?/i);
      if (m) {
        const catCount = parseInt(m[1], 10);
        if (catCount !== masterCount) warn(`SKILLS-CATALOG.md says ${catCount} skills, actual: ${masterCount}`);
...
if (runPhases.includes(27)) {
  ...
  if (fs.existsSync(m365Path)) {
    ...
    if (m365Pkg && vscPkg && m365Pkg.version !== vscPkg.version) {
      warn(`M365 heir version (${m365Pkg.version}) != VS Code heir (${vscPkg.version})`);
    } else if (m365Pkg && vscPkg) {
      pass(`M365 heir version aligned (${m365Pkg.version})`);
    }

    const m365Required = ['teamsapp.yml', path.join('appPackage', 'declarativeAgent.json'), 'README.md'];
    const m365Missing = m365Required.filter(f => !fs.existsSync(path.join(m365Path, f)));
    if (m365Missing.length > 0) m365Missing.forEach(f => fail(`M365 heir missing: ${f}`));
...
if (runPhases.includes(28)) {
  ...
  if (fs.existsSync(csPath)) {
    ...
    const csMissing = csRequired.filter(f => !fs.existsSync(path.join(csPath, f)));
    if (csMissing.length > 0) csMissing.forEach(f => fail(`Codespaces heir missing: ${f}`));
  ...
  if (fs.existsSync(gkPath)) {
    ...
    if (fs.existsSync(indexPath)) {
      ...
      if (index && index.entries) {
        const indexPatterns = index.entries.filter(e => e.type === 'pattern').length;
        const indexInsights = index.entries.filter(e => e.type === 'insight').length;
        const patternsDir = path.join(gkPath, 'patterns');
        const insightsDir = path.join(gkPath, 'insights');
        const actualPatterns = fs.existsSync(patternsDir) ? fs.readdirSync(patternsDir).filter(f => f.startsWith('GK-')).length : 0;
        const actualInsights = fs.existsSync(insightsDir) ? fs.readdirSync(insightsDir).filter(f => f.startsWith('GI-')).length : 0;

        if (indexPatterns !== actualPatterns) warn(`GK index patterns (${indexPatterns}) != actual files (${actualPatterns})`);
        if (indexInsights !== actualInsights) warn(`GK index insights (${indexInsights}) != actual files (${actualInsights})`);
        if (indexPatterns === actualPatterns && indexInsights === actualInsights) {
          pass(`GK index matches disk: ${actualPatterns} patterns, ${actualInsights} insights`);
        }
      }
    } else {
...
if (runPhases.includes(30)) {
  ...
  const missingMuscles = expectedMuscles.filter(m => !fs.existsSync(path.join(musclesDir, m)));
  if (missingMuscles.length > 0) missingMuscles.forEach(m => fail(`Missing muscle: ${m}`));
  else pass(`All trifecta-referenced muscles present (${expectedMuscles.length})`);

  const scriptsDir = path.join(rootPath, 'scripts');
  if (fs.existsSync(scriptsDir)) {
    const releaseScripts = ['release-vscode.ps1', 'release-preflight.ps1'];
    const missingRelease = releaseScripts.filter(s => !fs.existsSync(path.join(scriptsDir, s)));
    if (missingRelease.length > 0) missingRelease.forEach(m => warn(`Missing release script: ${m}`));
...
if (runPhases.includes(31)) {
  ...
  if (fs.existsSync(roadmapPath)) {
    ...
    if (rmMatch) {
      if (rmMatch[1] !== currentVersion) warn(`ROADMAP says master version ${rmMatch[1]}, package.json says ${currentVersion}`);
  ...
  if (fs.existsSync(configPath)) {
    ...
    if (config && config.version !== currentVersion) warn(`cognitive-config.json version (${config.version}) != ${currentVersion}`);
    else if (config) pass(`cognitive-config.json version matches (${currentVersion})`);
  }
  ...
  if (fs.existsSync(ciPath)) {
    ...
    if (ciMatch && ciMatch[1] !== currentVersion) warn(`copilot-instructions.md version (${ciMatch[1]}) != ${currentVersion}`);
    else if (ciMatch) pass(`copilot-instructions.md version matches (${currentVersion})`);
  }

  const clPath = path.join(rootPath, 'CHANGELOG.md');
  if (fs.existsSync(clPath)) {
    const clContent = fs.readFileSync(clPath, 'utf8');
    if (!clContent.includes(`[${currentVersion}]`)) warn(`CHANGELOG.md missing entry for ${currentVersion}`);
    else pass(`CHANGELOG.md has entry for ${currentVersion}`);
  }

  const heirClPath = path.join(rootPath, 'platforms', 'vscode-extension', 'CHANGELOG.md');
  if (fs.existsSync(heirClPath)) {
    const heirCl = fs.readFileSync(heirClPath, 'utf8');
    if (!heirCl.includes(`[${currentVersion}]`)) warn(`Heir CHANGELOG.md missing entry for ${currentVersion}`);
...
if (runPhases.includes(32)) {
  ...
  if (fs.existsSync(ciPath)) {
    ...
    if (!ciContent.includes('## Identity')) fail('copilot-instructions.md missing ## Identity section');
    else pass('Identity section present');

    // Agent list vs disk
    const agentFiles = fs.existsSync(path.join(ghPath, 'agents'))
      ? fs.readdirSync(path.join(ghPath, 'agents')).filter(f => f.endsWith('.agent.md'))
      : [];
    const diskAgents = ['Alex', ...agentFiles
    ...
    if (agentMatch) {
      const agentLine = agentMatch[1].split('\n')
        .filter(l => l.trim() && !l.trim().startsWith('<!--'))
        .shift() || '';
      const listed = agentLine.split(',').map(a => a.split('(')[0].trim()).filter(Boolean).sort();
      const missing = setDiff(diskAgents, listed);
      const extra = setDiff(listed, diskAgents);
      if (missing.length > 0) warn(`Agents on disk but NOT in copilot-instructions: ${missing.join(', ')}`);
      if (extra.length > 0) warn(`Agents in copilot-instructions but NOT on disk: ${extra.join(', ')}`);
      if (missing.length === 0 && extra.length === 0) pass(`Agent list matches disk (${diskAgents.length} agents)`);
    } else {
    ...
    if (trifMatch) {
      ...
      const missingSkills = listedNames.filter(n => !fs.existsSync(path.join(ghPath, 'skills', n)));

      if (listedCount !== listedNames.length) warn(`Trifecta count (${listedCount}) doesn't match listed names (${listedNames.length})`);
      else if (missingSkills.length > 0) warn(`Listed trifectas missing skill directories: ${missingSkills.join(', ')}`);
      else pass(`All ${listedCount} listed trifectas have skill directories`);

      // Instruction/prompt matching with aliases
      const instrAliases = {
        'dream-state': 'dream-state-automation', 'release-process': 'release-management',
        'research-first-development': 'research-first-workflow', 'brain-qa': 'semantic-audit',
        'architecture-audit': 'semantic-audit', 'code-review': 'code-review-guidelines',
        'global-knowledge': 'global-knowledge-curation', 'gamma-presentations': 'gamma-presentation'
      };
      const promptAliases = {
        'meditation': 'meditate', 'dream-state': 'dream', 'self-actualization': 'selfactualize',
        'release-process': 'release', 'brand-asset-management': 'brand',
        'research-first-development': 'gapanalysis', 'brain-qa': 'brainqa',
        'architecture-audit': 'masteraudit', 'bootstrap-learning': 'learn',
        'vscode-configuration-validation': 'validate-config', 'ui-ux-design': 'ui-ux-audit',
        'gamma-presentations': 'gamma', 'secrets-management': 'secrets',
        'chat-participant-patterns': 'chat-participant', 'vscode-extension-patterns': 'vscode-extension-audit',
        'mcp-development': 'mcp-server', 'microsoft-graph-api': 'graph-api',
        'teams-app-patterns': 'teams-app', 'm365-agent-debugging': 'm365-agent-debug',
        'testing-strategies': 'tdd', 'knowledge-synthesis': 'cross-domain-transfer',
        'north-star': 'northstar', 'md-to-word': 'word', 'debugging-patterns': 'debug',
        'refactoring-patterns': 'refactor', 'code-review': 'review',
        'global-knowledge': 'knowledge', 'ai-writing-avoidance': 'audit-writing'
      };

      const instrFiles = files(path.join(ghPath, 'instructions'), '.md').map(f => f.replace('.instructions.md', ''));
      const promptFiles = files(path.join(ghPath, 'prompts'), '.md').map(f => f.replace('.prompt.md', ''));

      const noInstr = listedNames.filter(n =>
        !instrFiles.includes(n) && !(instrAliases[n] && instrFiles.includes(instrAliases[n]))
      );
      const noPrompt = listedNames.filter(n =>
        !promptFiles.includes(n) && !(promptAliases[n] && promptFiles.includes(promptAliases[n]))
      );

      if (noInstr.length > 0) warn(`Listed trifectas with no matching instruction: ${noInstr.join(', ')}`);
      else pass('All listed trifectas have matching instructions');
      if (noPrompt.length > 0) warn(`Listed trifectas with no matching prompt: ${noPrompt.join(', ')}`);
      else pass('All listed trifectas have matching prompts');
    } else {
      warn('Could not parse trifecta list from copilot-instructions.md');
    }

    if (!ciContent.includes('## Active Context')) fail('copilot-instructions.md missing ## Active Context section');
    else pass('Active Context section present');
    if (!ciContent.includes('## User Profile')) fail('copilot-instructions.md missing ## User Profile section');
...
if (runPhases.includes(33)) {
  ...
  const missingFM = dirs(path.join(ghPath, 'skills')).filter(skill => {
    const skillMd = path.join(ghPath, 'skills', skill, 'SKILL.md');
    return fs.existsSync(skillMd) && !/^---\s*\n/.test(fs.readFileSync(skillMd, 'utf8'));
  });
  if (missingFM.length > 0) {
    fail(`Master has ${missingFM.length} skills without YAML frontmatter - will break heir`);
  ...
  const epDir = path.join(ghPath, 'episodic');
  if (fs.existsSync(epDir)) {
    const legacy = fs.readdirSync(epDir).filter(f => f.endsWith('.prompt.md'));
    if (legacy.length > 0) fail(`Master episodic has ${legacy.length} legacy .prompt.md files - archive before sync`);
  ...
  if (fs.existsSync(profilePath)) {
    ...
    if (prof && prof.name && prof.name.trim() !== '') {
      warn(`Master user-profile.json contains name: '${prof.name}' (will be excluded from heir)`);
      hasPII = true;
    }
    if (prof && prof.contact && prof.contact.email) {
      warn('Master user-profile.json contains email (will be excluded from heir)');
      hasPII = true;
    }
  ...
  for (const skill of dirs(path.join(ghPath, 'skills'))) {
    ...
    for (const conn of syn.connections) {
      const t = conn.target;
      if (!t || /^(external:|global-knowledge:\/\/|https?:\/\/)/.test(t)) continue;
      let fullPath;
      if (/^\.github\//.test(t)) fullPath = path.join(rootPath, t);
      else if (t.startsWith('../')) fullPath = path.resolve(path.join(ghPath, 'skills', skill), t);
      else fullPath = path.join(ghPath, 'skills', skill, t);
      if (!fs.existsSync(fullPath)) brokenRefs.push(`${skill}: ${t}`);
    }
  }
  if (brokenRefs.length > 0) {
    fail(`Master has ${brokenRefs.length} broken synapse references - fix before sync`);
    brokenRefs.slice(0, 10).forEach(r => fail(`  ${r}`));
    if (brokenRefs.length > 10) fail(`  ... and ${brokenRefs.length - 10} more`);
  } else {
  ...
  const exclusions = [
    'heir-sync-management', 'm365-agent-debugging', 'teams-app-patterns', 'azure-devops-automation',
    'chat-participant-patterns', 'vscode-configuration-validation', 'vscode-extension-patterns',
    'azure-architecture-patterns', 'enterprise-integration', 'persona-detection'
  ];
  const contaminated = [];
  for (const skill of dirs(path.join(ghPath, 'skills'))) {
...
if (runPhases.includes(34)) {
  ...
  function testSelfContained(sourceFile, target) {
    ...
    if (/^(https?:\/\/|#|mailto:|external:|global-knowledge:\/\/)/.test(target)) return null;
    ...
    try {
      ...
      for (const conn of j.connections) {
        ...
        if (escaped !== null) {
          ...
          if (!isExcepted(short)) scIssues.push(`[synapse] ${short} -> ${escaped}`);
        }
      }
  ...
  const synapseCount = findRecursive(ghPath, n => n === 'synapses.json').length;
  const mdCount = findRecursive(ghPath, n => n.endsWith('.md')).length;
  if (scIssues.length === 0) pass(`Brain is fully self-contained (${synapseCount} synapse files, ${mdCount} markdown files scanned)`);
...
if (runPhases.includes(35)) {
  ...
  const keyMap = {};
  for (const f of findRecursive(path.join(ghPath, 'skills'), n => n === 'synapses.json')) {
    const j = readJSON(f);
    if (!j || !j.apiKeys) continue;
    const skill = path.basename(path.dirname(f));
    for (const req of j.apiKeys) {
      const k = req.envVar;
      if (!keyMap[k]) keyMap[k] = { skills: [], purpose: req.purpose, required: req.required, getUrl: req.getUrl };
  ...
  if (Object.keys(keyMap).length === 0) {
    ...
    for (const envVar of Object.keys(keyMap).sort()) {
      ...
      if (process.env[envVar]) {
        ...
        warn(`${envVar} not set (${tag}) — needed by: ${entry.skills.join(', ')} | ${entry.purpose}${hint}`);
        missingKeys.push(envVar);
      }
    }
    if (missingKeys.length === 0) pass(`All ${Object.keys(keyMap).length} declared API key(s) are present`);
    else {
      warn(`${missingKeys.length} API key(s) missing — affected skills will fail at runtime`);
      warn('Keys may also be stored in VS Code SecretStorage (not checkable from Node.js)');
    }
  }
}
