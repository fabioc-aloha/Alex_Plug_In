...
| Concept | Definition |
|---------|-----------|
| **Heir** | Platform-specific deployment inheriting Master's DNA (VS Code, M365, Agent Plugin) |
...
| **Deployment Heir** | Heir needing only configuration, no code translation (e.g., Agent Plugin — `.github/` plugin files) |
...
| **Agent Plugin** | Deployment | None (plugin files only) | `git commit` + push / plugin update | Very Low |
...
| Agent Plugin | GitHub Copilot (untuned) | "AI assistants" generically |
