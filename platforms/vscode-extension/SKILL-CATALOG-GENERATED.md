# Alex Skills Catalog (Generated)

> Generated: 2026-04-12
> Total Skills: **161** (147 inheritable, 5 master-only, 7 VS Code, 2 M365, 0 temporary)
> Total Connections: **830** (0 bidirectional, 136 weak)

---

## Skills by Inheritance

### Inheritable (147)

| Skill | Connections |
| ----- | ----------- |
| `academic-research` | 8 |
| `agent-debug-panel` | 4 |
| `ai-agent-design` | 9 |
| `ai-character-reference-generation` | 4 |
| `ai-generated-readme-banners` | 6 |
| `ai-writing-avoidance` | 3 |
| `airs-appropriate-reliance` | 5 |
| `alex-effort-estimation` | 4 |
| `anti-hallucination` | 6 |
| `api-design` | 5 |
| `api-documentation` | 5 |
| `appropriate-reliance` | 5 |
| `architecture-audit` | 7 |
| `architecture-health` | 6 |
| `architecture-refinement` | 5 |
| `ascii-art-alignment` | 4 |
| `awareness` | 6 |
| `azure-deployment-operations` | 2 |
| `azure-openai-patterns` | 5 |
| `bicep-avm-mastery` | 1 |
| `book-publishing` | 3 |
| `bootstrap-learning` | 5 |
| `brain-qa` | 13 |
| `brand-asset-management` | 9 |
| `business-analysis` | 6 |
| `career-development` | 5 |
| `change-management` | 5 |
| `character-aging-progression` | 3 |
| `chart-interpretation` | 6 |
| `citation-management` | 1 |
| `coaching-techniques` | 1 |
| `code-review` | 10 |
| `cognitive-load` | 3 |
| `cognitive-symbiosis` | 3 |
| `comedy-writing` | 4 |
| `content-safety-implementation` | 5 |
| `converter-qa` | 3 |
| `correax-brand` | 6 |
| `counseling-psychology` | 5 |
| `creative-writing` | 5 |
| `cross-cultural-collaboration` | 6 |
| `dashboard-design` | 7 |
| `data-analysis` | 8 |
| `data-quality-monitoring` | 4 |
| `data-storytelling` | 8 |
| `data-visualization` | 7 |
| `database-design` | 6 |
| `debugging-patterns` | 3 |
| `deep-work-optimization` | 1 |
| `dialog-engineering` | 4 |
| `dissertation-defense` | 1 |
| `distribution-security` | 3 |
| `doc-hygiene` | 4 |
| `documentation-quality-assurance` | 2 |
| `docx-to-md` | 3 |
| `dream-state` | 4 |
| `error-recovery-patterns` | 3 |
| `executive-storytelling` | 1 |
| `fabric-notebook-publish` | 4 |
| `financial-analysis` | 5 |
| `flux-brand-finetune` | 8 |
| `foundry-agent-platform` | 7 |
| `frustration-recognition` | 1 |
| `game-design` | 5 |
| `gamma-presentations` | 6 |
| `git-workflow` | 2 |
| `global-knowledge` | 7 |
| `global-knowledge-sync` | 5 |
| `grant-writing` | 4 |
| `graphic-design` | 9 |
| `healthcare-informatics` | 5 |
| `heir-bootstrap` | 5 |
| `heir-feedback` | 4 |
| `hr-people-operations` | 5 |
| `image-handling` | 9 |
| `incident-response` | 3 |
| `infrastructure-as-code` | 5 |
| `journalism` | 5 |
| `knowledge-synthesis` | 7 |
| `learning-psychology` | 3 |
| `legal-compliance` | 5 |
| `lint-clean-markdown` | 4 |
| `literature-review` | 1 |
| `llm-model-selection` | 4 |
| `localization` | 10 |
| `markdown-mermaid` | 7 |
| `mcp-development` | 8 |
| `md-scaffold` | 4 |
| `md-to-eml` | 3 |
| `md-to-html` | 4 |
| `md-to-word` | 7 |
| `meditation` | 5 |
| `meeting-efficiency` | 5 |
| `memory-activation` | 5 |
| `memory-curation` | 7 |
| `memory-export` | 4 |
| `microsoft-fabric` | 5 |
| `microsoft-graph-api` | 4 |
| `msal-authentication` | 4 |
| `multi-agent-orchestration` | 7 |
| `muscle-memory-recognition` | 5 |
| `nav-inject` | 3 |
| `north-star` | 9 |
| `observability-monitoring` | 6 |
| `performance-profiling` | 6 |
| `pii-privacy-regulations` | 6 |
| `post-mortem` | 6 |
| `pptx-generation` | 4 |
| `practitioner-research` | 7 |
| `presentation-tool-selection` | 4 |
| `privacy-responsible-ai` | 5 |
| `proactive-assistance` | 7 |
| `project-deployment` | 4 |
| `project-management` | 7 |
| `project-scaffolding` | 7 |
| `prompt-engineering` | 6 |
| `prompt-evolution-system` | 5 |
| `rag-architecture` | 7 |
| `react-vite-performance` | 4 |
| `refactoring-patterns` | 5 |
| `research-first-development` | 10 |
| `research-project-scaffold` | 5 |
| `root-cause-analysis` | 4 |
| `rubber-duck-debugging` | 7 |
| `sales-enablement` | 5 |
| `scope-management` | 6 |
| `secrets-management` | 7 |
| `security-review` | 9 |
| `self-actualization` | 6 |
| `service-worker-offline-first` | 4 |
| `silence-as-signal` | 5 |
| `skill-building` | 1 |
| `skill-development` | 9 |
| `slide-design` | 1 |
| `socratic-questioning` | 6 |
| `sse-streaming` | 4 |
| `stakeholder-management` | 6 |
| `status-reporting` | 6 |
| `svg-graphics` | 5 |
| `terminal-image-rendering` | 3 |
| `testing-strategies` | 9 |
| `text-to-speech` | 4 |
| `token-waste-elimination` | 8 |
| `ui-ux-design` | 7 |
| `visual-memory` | 6 |
| `vscode-environment` | 4 |
| `work-life-balance` | 5 |

### Master-Only (5)

| Skill | Connections |
| ----- | ----------- |
| `extension-audit-methodology` | 6 |
| `heir-sync-management` | 5 |
| `release-preflight` | 8 |
| `release-process` | 10 |
| `skill-catalog-generator` | 4 |

### VS Code Heir (7)

| Skill | Connections |
| ----- | ----------- |
| `azure-architecture-patterns` | 1 |
| `azure-devops-automation` | 1 |
| `chat-participant-patterns` | 3 |
| `enterprise-integration` | 3 |
| `persona-detection` | 5 |
| `vscode-configuration-validation` | 6 |
| `vscode-extension-patterns` | 14 |

### M365 Heir (2)

| Skill | Connections |
| ----- | ----------- |
| `m365-agent-debugging` | 2 |
| `teams-app-patterns` | 4 |

---

## Staleness-Prone Skills

> These skills depend on external APIs or platforms that change frequently.

| Skill | Reason |
| ----- | ------ |
| `vscode-extension-patterns` | Platform/API changes frequently |
| `chat-participant-patterns` | Platform/API changes frequently |
| `m365-agent-debugging` | Platform/API changes frequently |
| `teams-app-patterns` | Platform/API changes frequently |
| `llm-model-selection` | Platform/API changes frequently |
| `git-workflow` | Platform/API changes frequently |
| `privacy-responsible-ai` | Platform/API changes frequently |
| `microsoft-sfi` | Platform/API changes frequently |

---

_Alex Cognitive Architecture -- Take Your Code to New Heights_
